...
Since 2026-09-02 this scope is part of the **default** set issued by `scripts/connect-agents.sh` (`decisions,knowledge,inbox,error-patterns,task-board,personal,projects,daily`), so a freshly connected agent already has it. (`external` was folded into `knowledge` by migration `011_retire_unused_scopes.sql`, which also updated the default set.) The SQL below is only for topping up an agent issued before that:
