...
Пользователь, под которым работает агент, этот `.env` прочитать не может, поэтому `install.sh` ставит ещё и root-хелпер `/usr/local/sbin/labops-issue-agent-token <agent-id> [scopes]` с узким правилом sudo: он проверяет имя агента, принимает только канонические скоупы и отказывает на `*`. Его зовёт скилл `create-agent` — без этого свежий агент поднимался с `CHANGE_ME` вместо токена.
...
- Если на машине **уже есть агенты** (поставлен [`labops-agent-architecture`](#часть-labops)) — установщик подключает их автоматически через `scripts/connect-agents.sh`. Для каждого воркспейса под `~/.claude-lab/*/.claude` он: выдаёт реальный токен взамен плейсхолдера `CHANGE_ME` (а также если токена нет в БД или ему не хватает базовых прав), дописывает в `.mcp.json` сервисы, которых там нет (у агентов прежних выпусков не было доски задач), доливает недостающие `SECOND_BRAIN_*_URL` в `agent.env` объединяет прежние scopes (из БД, если их удалось прочитать) с базовым набором, а если токен оставлен — выравнивает отставшую строку `AGENT_SCOPES` по БД, без перезапуска агента. Бэкапы — `*.bak-connect`. Перед перезапуском сессии агента скрипт дожидается, пока все четыре MCP-сервиса ответят (`memory_router` греет модель эмбеддингов и слушает порт не сразу) — иначе сессия подключилась бы мимо них. Отключить: `SKIP_AGENT_CONNECT=1`; не ждать сервисы: `SKIP_SERVICE_WAIT=1`; не трогать scopes: `KEEP_AGENT_SCOPES=1`; повторить отдельно: `sudo bash scripts/connect-agents.sh`.
