...
flowchart LR
    ...
    L2["L2 · Active<br/>active/episodic.md, working-set.md, handoff.md<br/>(current work)"]
    L3["L3 · Passive<br/>passive/decisions.md, errors.md, insights.md<br/>(consolidation)"]
...
- If **agents already exist** on the machine (`labops-agent-architecture` is installed) — the installer auto-connects them via `scripts/connect-agents.sh`. For every workspace under `~/.claude-lab/*/.claude` it: issues a real token when the bearer is still `CHANGE_ME`, absent from the DB, or short of the baseline scopes; adds MCP servers missing from `.mcp.json` (agents scaffolded before the task board have no `second_brain-tasks` entry); appends the missing `SECOND_BRAIN_*_URL` vars to `agent.env`; merges the agent's existing scopes (read from the DB when available) with the default set; and, when the token is kept, rewrites a stale `AGENT_SCOPES` line to match the DB — without restarting the agent. Backups land next to each file as `*.bak-connect`. Before restarting an agent session it waits for all four MCP services to answer (`memory_router` warms the embedding model and binds its port late) — restarting earlier would leave the session permanently attached to nothing. Opt out with `SKIP_AGENT_CONNECT=1`, skip the wait with `SKIP_SERVICE_WAIT=1`, keep scopes untouched with `KEEP_AGENT_SCOPES=1`; re-run standalone any time: `sudo bash scripts/connect-agents.sh`.
